...
public static class ValueExtensions
{
    ...
    {
        var identifierSpan = baseIdentifier.AsSpan()[(baseIdentifier.IndexOf(':', StringComparison.Ordinal) + 1)..];
        return entityType switch
        {
            EntityType.Cover => baseIdentifier.StartsWith("COVER:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"COVER:{identifierSpan}",
            EntityType.Button => baseIdentifier.StartsWith("BUTTON:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"BUTTON:{identifierSpan}",
            EntityType.Climate => baseIdentifier.StartsWith("CLIMATE:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"CLIMATE:{identifierSpan}",
            EntityType.Light => baseIdentifier.StartsWith("LIGHT:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"LIGHT:{identifierSpan}",
            EntityType.MediaPlayer => baseIdentifier.Length != identifierSpan.Length ? identifierSpan.ToString() : baseIdentifier,
            EntityType.Remote => baseIdentifier.StartsWith("REMOTE:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"REMOTE:{identifierSpan}",
            EntityType.Sensor => baseIdentifier.StartsWith("SENSOR:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"SENSOR:{identifierSpan}",
            EntityType.Switch => baseIdentifier.StartsWith("SWITCH:", StringComparison.OrdinalIgnoreCase) ? baseIdentifier : $"SWITCH:{identifierSpan}",
