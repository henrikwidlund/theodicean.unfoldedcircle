...
internal sealed class CustomSystemdConsoleFormatter : ConsoleFormatter, IDisposable
{
    private readonly IDisposable? _optionsReloadToken;

    public CustomSystemdConsoleFormatter(IOptionsMonitor<ConsoleFormatterOptions> options)
        : base("CustomSystemd")
    {
        ...
        int eventId, string? exception, DateTimeOffset stamp)
    {
        ...
        {
            textWriter.Write(stamp.ToString(timestampFormat, DateTimeFormatInfo.InvariantInfo));
        }
    ...
    private static string GetSyslogSeverityString(LogLevel logLevel)
    {
        ...
        {
            LogLevel.Trace => "<6>",
            LogLevel.Debug => "<6>",        // debug-level messages
            LogLevel.Information => "<5>",  // informational messages
